        ...
        for orderer in orderers:
            ...
            for index in range(int(settings.NUM_ORDERERS)):
                orderer_cmd += (''
                    ...
                    'cp ./orgs/'+domain+'/msp/signcerts/cert.pem crypto-config/ordererOrganizations/'+domain+'/orderers/orderer'+str(index)+'-'+orderer+'.'+domain+'/msp/signcerts/;'
                    'cp ./orgs/'+domain+'/msp/keystore/key.pem crypto-config/ordererOrganizations/'+domain+'/orderers/orderer'+str(index)+'-'+orderer+'.'+domain+'/msp/keystore/key.pem;'
                    'cp ./orgs/'+domain+'/tls/server.crt crypto-config/ordererOrganizations/'+domain+'/orderers/orderer'+str(index)+'-'+orderer+'.'+domain+'/tls/;'
                    'cp ./orgs/'+domain+'/tls/server.key crypto-config/ordererOrganizations/'+domain+'/orderers/orderer'+str(index)+'-'+orderer+'.'+domain+'/tls/;'
                '')
            orderer_cmd += (''
